...
class Dropdown(discord.ui.Select):
    def __init__(self, ctx: Union[Context, discord.Interaction], bot: SkyeBot, member):
        ...
        options = [
            discord.SelectOption(
                ...
                description=f"Shows information about: {self.member}",
                emoji="<:information:777129869862633472>",
            ),
            discord.SelectOption(
                label="Account Avatar",
                description=f"Shows the avatar of: {self.member}",
                emoji="<:tpose:816281552576577536>",
            ),
        ]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "Account Avatar":
            ...
            embed = discord.Embed(
                description=f"{self.member}'s In-Game Avatar", color=0x3867A8
            )
            embed.set_image(url=user_thumbnail.image_url)
            await interaction.message.edit(
                embed=embed, view=DropdownView(interaction, self.bot, self.member)
            )
        ...
        if self.values[0] == "Account Information":
            ...
            embed.add_field(
                name="Description", value=f"**{description!r}**", inline=False
            )
...
class DropdownView(discord.ui.View):
    def __init__(
        self, ctx: Union[Context, discord.Interaction], bot: SkyeBot, member=None
    ):
        super().__init__()
        self.ctx = ctx
        self.bot = bot
