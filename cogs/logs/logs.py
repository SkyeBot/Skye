...
class Logging(commands.Cog):
    ...
    logging = app_commands.Group(
        name="logging",
        description="All logging commands",
    ...
    async def _enable(self, interaction: discord.Interaction, channel: Optional[discord.TextChannel]):
        ...
        try:
            ...
            if exists is None:
                ...
                em = discord.Embed(title="", color=discord.Color(3342080))
                em.add_field(name="Logging currently enabled! ", value=f"   Logs are in: {channel.mention}")

            else:
                await self.bot.pool.execute("UPDATE logs SET channel_id = $1 WHERE guild_id = $2", channel.id, interaction.guild.id)

                em = discord.Embed(title="", color=discord.Color(3342080))
                em.add_field(name="Logging channel Updated!", value=f"New Channel: {channel.mention}")
    ...
    async def on_message_delete(self, message: discord.Message):
        ...
        try:
            exists = await self.bot.pool.fetchval(
                "SELECT channel_id FROM LOGS WHERE guild_id = $1", message.guild.id
            )
            channel = self.bot.get_channel(exists)
            if exists is not None and message.author is not message.author.bot:
                ...
                if message.content:
                    ...
                    deleted.add_field(
                        name="ID:",
                        value=f"```User = {message.author.id}\nMessage = {message.id}```",
                    )
                ...
                if message.attachments:
                    ...
                    if len(file_type) != 1 and file_type[-1] in [
                        "png",
                        "jpeg",
                        "gif",
                        "webp",
                        ...
                        req = await http.get(
                            file.proxy_url, res_method="read", no_cache=True
                        )
                        ...
                        deleted.add_field(
                            name="Deleted Image:",
                            value=f"[{file.filename}]({file.proxy_url})",
                            inline=False,
                        )
                        deleted.add_field(
                            name="ID:",
    ...
    async def on_message_edit(self, before: discord.Message, after: discord.Message):
        ...
        exists = await self.bot.pool.fetchrow(
            "SELECT channel_id FROM LOGS WHERE guild_id = $1", before.guild.id
        )
        ...
        embed.add_field(
            name="Before:", value=before.content or "No Content Available", inline=False
        )
        embed.add_field(name="After:", value=after.content, inline=False)
        embed.add_field(
            name="ID:",
            value=f"```User = {before.author.id} \nMessage = {before.id}```",
            inline=False,
        )
    ...
    async def on_member_ban(self, guild: discord.Guild, user: discord.Member):
        try:
            ...
            show_roles = ", ".join([f"<@&{x.id}>" for x in sorted(user.roles, key=lambda x: x.position, reverse=True) if x.id != user.guild.default_role.id]) if len(user.roles) > 1 else "None"

            embed = discord.Embed(description=f"Member {user.mention} was banned!")
    ...
    async def on_member_unban(self, guild, user: discord.Member):
        try:
            ...
            channel = self.bot.get_channel(exists.get("channel_id"))
            embed = discord.Embed(description=f"User: {user.mention} was unbanned!")
            embed.set_author(name=user, icon_url=user.avatar)
            embed.add_field(name="ID:", value=f"```User = {user.id} \n```", inline=False)
            embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
            await channel.send(embed=embed)
    ...
    async def on_guild_role_delete(self, role: discord.Role):
        try:
            ...
            async for entry in role.guild.audit_logs(action=discord.AuditLogAction.role_delete):
                if entry.id == role.id:
                    embed.add_field(name="Responsible Moderator", value=f"{entry.user}")
            embed.add_field(name="ID of Role:", value=f"{role.id}")
            embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
            await channel.send(embed=embed)
        except:
    ...
    async def on_member_join(self, member: discord.Member):
        try:
            ...
            channel = self.bot.get_channel(exists.get("channel_id"))
            show_roles = ", ".join([f"<@&{x.id}>" for x in sorted(member.roles, key=lambda x: x.position, reverse=True) if x.id != member.guild.default_role.id]) if len(member.roles) > 1 else "None"

            embed = discord.Embed(title=f"New member!\nThis server is now at {len(member.guild.members)} Members", description=f"Member username: {member}", timestamp=datetime.datetime.now(datetime.timezone.utc), color=discord.Color.dark_green())

            embed.add_field(name="Join server date: ", value=default.date(member.joined_at), inline=False)
    ...
    async def on_member_remove(self, member: discord.Member):
        try:
            ...
            embed = discord.Embed(title=f"A Member Left!\nThis server is now at {len(member.guild.members)} Members!", description=f"Member username: {member}", timestamp=datetime.datetime.now(datetime.timezone.utc), color=discord.Color.brand_red())

            embed.add_field(name="Left Server: ", value=default.date(datetime.datetime.now(datetime.timezone.utc), ago=True), inline=False)
    ...
    async def on_guild_channel_update(self, before, after):
        try:
            ...
            if before.category != after.category and after.category is not None:
                ...
                embed.add_field(name="Old category:", value=before.category, inline=False)
