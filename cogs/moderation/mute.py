...
class Mute(commands.Cog):
    ...
    async def set_mute(self, interaction: discord.Interaction, role: discord.Role):
        exists = await self.bot.pool.fetchrow(
            "SELECT role_id FROM MUTE_CONFIG WHERE guild_id = $1", interaction.guild.id
        )
        try:

            if exists == None:
                await self.bot.pool.execute(
                    "INSERT INTO mute_config(role_id, guild_id) VALUES ($1, $2)",
                    role.id,
                    interaction.guild.id,
                )
                em = discord.Embed(title="", color=discord.Color(0x32FF00))
                em.add_field(
                    name="Mute Config Setup", value=f"Mute Role: {role.mention}"
                )
                await interaction.response.send_message(embed=em)
            else:
                await self.bot.pool.execute(
                    "UPDATE mute_config SET role_id = $1 WHERE guild_id = $2",
                    role.id,
                    interaction.guild.id,
                )
                em = discord.Embed(title="", color=discord.Color(0x32FF00))
                em.add_field(
                    name="Mute Config Updated", value=f"New Mute Role: {role.mention}"
                )
                await interaction.response.send_message(embed=em)
    ...
    async def mute_slash(
        ...
        try:
            ...
            role_query = await self.bot.pool.fetchval(
                "SELECT role_id FROM MUTE_CONFIG WHERE guild_id = $1",
                interaction.guild.id,
            )
            ...
            embed = discord.Embed(
                title="❌ Muted",
                description=f"{interaction.user.mention} Muted {member.mention}",
            ...
            await member.send(
                f"You have been muted from {interaction.guild.name}\nReason: {reason}"
            )
