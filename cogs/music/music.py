...
from utils.paginator import MusicPages
...
class Music(commands.Cog):
    ...
    async def connect_nodes(self):
        ...
        await wavelink.NodePool.create_node(
            bot=self.bot, host="lavalink", port=2333, password="youshallnotpass"
        )
    ...
    async def play(self, interaction: discord.Interaction, *, url: str):
        ...
        if f"{interaction.guild.id}" not in self.muloop.keys():
            if vc.queue.is_empty and not vc.is_playing():
                await vc.play(song)
                embed = discord.Embed(
                    description=f"Now playing: **[{song.title}]({song.uri}) By {song.author}**"
                )
                embed.set_author(
            ...
            return await interaction.response.send_message(
                f"Added `{song.title} - {song.author}` to the queue..."
            )
        ...
        if "https://www.youtube.com/playlist?list=" in url:
            ...
            await interaction.followup.send(
                f"Added {len(playlist.tracks[:10])} songs to the queue\nNow Playing: {new_track.title}"
            )
    ...
    async def loop(self, interaction: discord.Interaction):
        ...
        if f"{interaction.guild.id}" not in self.muloop.keys():
            ...
            await interaction.response.send_message(
                f"Now looping: {cp.title} - {cp.author}"
            )
        else:
    ...
    async def cp(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Currently playing: {vc.source.uri}\n{vc.source.title}"
        )
    ...
    async def clear(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Cleared {len(vc.queue)} songs from the queue."
        )
        vc.queue.clear()
    ...
    async def volume(self, interaction: discord.Interaction, volume: int):
        ...
        if not vc:
            ...
            return await interaction.response.send_message(
                "You cannot put the volume over 100!"
            )
    ...
    async def join(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"Joined: <#{interaction.user.voice.channel.id}>"
        )
