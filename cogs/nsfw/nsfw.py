...
class NSFW(commands.Cog):
    ...
    async def nsfw(self, interaction: discord.Interaction, *, endpoints: str):
        ...
        async with self.bot.session.get(
            f"https://i.thino.pics/{data.filename}"
        ) as resp:
            if resp.status == 404:
                return await interaction.response.send_message(
                    "No image was found in this endpoint!"
                )
            ...
            await interaction.response.send_message(
                embed=embed, file=discord.File(bio, filename=data.filename)
            )

    @app_commands.command(
        name="tomboy", description=f"Gets tomboy images from thino.pics", nsfw=True
    )
    ...
    @app_commands.command(
        name="helltaker", description=f"Gets helltaker porn from thino.pics", nsfw=True
    )
    ...
    async def search(self, interaction: discord.Interaction, image: str):
        ...
        print(finished_url)
        embed = discord.Embed(
            description=f"Found the file name: [{image}]({finished_url}) on the endpoint: [{url_endpoint}]({url})",
