...
class NSFW(commands.Cog):
    ...
    async def fruit_autocomplete(self, interaction: discord.Interaction,current: str,) -> List[app_commands.Choice[str]]:
        return [
            ...
            for choices in self.ENDPOINTS if current.lower() in choices.lower()
        ]

    @app_commands.command(description="An NSFW command",nsfw=True)
    @app_commands.autocomplete(choices=fruit_autocomplete)
    async def nsfw(self, interaction: discord.Interaction, *, choices: str):
        if choices not in self.ENDPOINTS:
