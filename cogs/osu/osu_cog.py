...
class UserSelect(discord.ui.Select):
    def __init__(self, ctx: Union[discord.Interaction, Context],user: User):
        ...
        options = [
            discord.SelectOption(label='Favorite Beatmaps', value="bea",description=f"Gets {self.user.username}'s favorite beatmaps"),
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "bea":
            a = await interaction.client.osu.fetch_user_beatmaps(self.user.id, "favourite", 5)
            n = [Beatmap(a) for a in a]
            self.embed.title = f"{self.user.username}'s favourite beatmaps"
            self.embed.description = '\n'.join(f"{beatmap.title} - {beatmap.artist}" for beatmap in n)
...
class DropdownView(discord.ui.View):
    def __init__(self, ctx: Union[Context, discord.Interaction], user: User):
        super().__init__()
        self.ctx = ctx
        self.user = user
