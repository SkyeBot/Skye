    ...
    def __init__(self, ctx: Union[discord.Interaction, Context], user: User):
        ...
        options = [
            discord.SelectOption(
                label="Beatmaps",
                value="bea",
                description=f"Beatmaps on {self.user.username}'s plays/has",
            ),
            discord.SelectOption(
                label="Account Avatar",
                description=f"Shows the avatar of: {self.user.username}",
            ),
            discord.SelectOption(
                label="Info", description=f"Info about: {self.user.username}"
            ),
            discord.SelectOption(
                label="Statistics", description=f"Statistics about {self.user.username}"
            ),
        ]

        super().__init__(
            min_values=1, max_values=1, options=options, custom_id="OsuSelectID"
        )
...
class osu(commands.Cog):
    ...
    set = app_commands.Group(
        name="set", description="allows you to set various things for osu", parent=osu
    )
    ...
    async def user(self, interaction: discord.Interaction, username: Optional[str]):
        ...
        try:
            user_query = await self.bot.pool.fetchrow(
                "SELECT osu_username FROM osu_user WHERE user_id = $1",
                interaction.user.id,
            )
    ...
    async def user(self, interaction: discord.Interaction, username: str):
        ...
        try:
            ...
            await interaction.response.send_message(
                f"Sucessfullly set your osu username to: {username}"
            )
