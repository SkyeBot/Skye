...
class SuggestModal(discord.ui.Modal):
    ...
    feature = discord.ui.TextInput(
        label="Suggest the feature here", style=discord.TextStyle.long
    )
    ...
    async def on_submit(self, interaction: discord.Interaction) -> None:
        ...
        embed = discord.Embed(
            description=f"New Feature Suggested!\nSuggested By {interaction.user}"
        )
        embed.add_field(name="Feature", value=self.feature.value)
        await channel.send(
            "<@894794517079793704> New Feature Was Suggested", embed=embed
        )

        await interaction.response.send_message(
            f"Thank you for suggesting a feature {interaction.user.mention}!",
            ephemeral=True,
