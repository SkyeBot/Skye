...
async def main():
    async with aiohttp.ClientSession() as session, asyncpraw.Reddit(
        client_id=os.environ["REDDIT_CLIENT_ID"],
        client_secret=os.environ["REDDIT_CLIENT_SECRET"],
        user_agent=os.environ["REDDIT_USER_AGENT"],
    ) as reddit, asyncpg.create_pool(os.environ["POSTGRES_URI"]) as pool, SkyeBot(
        session=session,
        ...
        async def blacklist_check(interaction: discord.Interaction):
            ...
            if await blacklist.check(bot, interaction.user):
                ...
                embed.add_field(
                    name="You are blacklisted from using this bot!",
                    value="If you think this is a mistake, go ahead and make an appeal at discord.gg/fp9HwavUQz",
                )
